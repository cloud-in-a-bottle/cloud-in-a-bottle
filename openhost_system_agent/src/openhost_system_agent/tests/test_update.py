...
# ── Auto-track the checked-out dev branch ────────────────────────────


def test_get_remote_info_adopts_non_main_branch_when_unpinned(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    # Unpinned, but HEAD is on a dev branch -> track it: report it AND persist the pin.
    remote = _make_repo(tmp_path / "remote", ["v1.0.0", "v1.1.0"])
    _branch(remote, "dev", "dev work")
    local = _clone_at(remote, tmp_path / "local", checkout="dev")
    monkeypatch.setattr(update_mod, "_repo", lambda: local)

    info = update_mod.get_remote_info()

    assert info.ref == "dev" and info.pinned is True
    assert update_mod._get_target_ref(local) == "dev"  # adopted


def test_get_remote_info_adopts_branch_on_pin_mismatch(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    # Pinned to main but actually checked out on a dev branch -> the branch wins.
    remote = _make_repo(tmp_path / "remote", ["v1.0.0"])
    _branch(remote, "dev", "dev work")
    local = _clone_at(remote, tmp_path / "local", checkout="dev")
    update_mod._set_target_ref(local, "main")  # stale pin
    monkeypatch.setattr(update_mod, "_repo", lambda: local)

    info = update_mod.get_remote_info()

    assert info.ref == "dev"
    assert update_mod._get_target_ref(local) == "dev"  # switched main -> dev


def test_unpinned_on_main_stays_tag_tracking(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    # Production case: on main, unpinned -> NOT adopted; still tag-tracking, unpinned.
    remote = _make_repo(tmp_path / "remote", ["v1.0.0", "v1.1.0"])
    local = _clone_at(remote, tmp_path / "local")  # HEAD on main, unpinned
    monkeypatch.setattr(update_mod, "_repo", lambda: local)

    info = update_mod.get_remote_info()

    assert info.pinned is False
    assert update_mod._get_target_ref(local) is None  # left alone


def test_detached_at_tag_is_not_adopted(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    # Detached HEAD (tag checkout) -> nothing to adopt; stays unpinned/tag-tracking.
    remote = _make_repo(tmp_path / "remote", ["v1.0.0", "v1.1.0"])
    local = _clone_at(remote, tmp_path / "local", checkout="v1.0.0")
    monkeypatch.setattr(update_mod, "_repo", lambda: local)

    update_mod.get_remote_info()

    assert update_mod._get_target_ref(local) is None


def test_fetch_updates_tracks_current_branch_when_it_advances(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    # On a dev branch (unpinned): fetch compares against the branch tip, not the latest tag.
    remote = _make_repo(tmp_path / "remote", ["v1.0.0"])
    _branch(remote, "dev", "d1")
    local = _clone_at(remote, tmp_path / "local", checkout="dev")
    monkeypatch.setattr(update_mod, "_repo", lambda: local)
    assert update_mod.fetch_updates().state == "UP_TO_DATE"

    _git(remote, "checkout", "dev")
    _commit(remote, "d2")  # advance the remote branch
    assert update_mod.fetch_updates().state == "BEHIND_REMOTE"


def test_apply_update_from_dev_branch_persists_pin_before_walk(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    # apply_update must persist the dev branch to the pin so the (separate, HEAD-detaching) walk
    # process tracks it.
    remote = _make_repo(tmp_path / "remote", ["v1.0.0"])
    _branch(remote, "dev", "dev work")
    local = _clone_at(remote, tmp_path / "local", checkout="dev")
    ...
    assert update_mod._get_target_ref(local) == "dev"  # adopted before handoff
    assert len(calls) == 1  # handed off to the walk
