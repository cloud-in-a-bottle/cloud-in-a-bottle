"""Unit tests for the ciab.toml manifest parser."""
...
class TestManifestFileResolution:
    """``ciab.toml`` is the canonical manifest name; ``openhost.toml`` is
    ...
    def test_canonical_name_is_ciab_toml(self):
        """The preferred/canonical filename must be ciab.toml."""
        assert MANIFEST_FILENAMES[0] == "ciab.toml"
    ...
    def test_find_returns_ciab_toml(self, tmp_path):
        (tmp_path / "ciab.toml").write_text(MINIMAL)
        assert find_manifest_path(str(tmp_path)) == str(tmp_path / "ciab.toml")
    ...
    def test_find_prefers_ciab_when_both_present(self, tmp_path):
        (tmp_path / "ciab.toml").write_text(MINIMAL)
        (tmp_path / "openhost.toml").write_text(MINIMAL)
        assert find_manifest_path(str(tmp_path)) == str(tmp_path / "ciab.toml")
    ...
    def test_parse_reads_ciab_toml(self, tmp_path):
        (tmp_path / "ciab.toml").write_text(MINIMAL)
    ...
    def test_parse_prefers_ciab_over_openhost(self, tmp_path):
        """When both files exist, ciab.toml is the one that gets parsed."""
        ciab = MINIMAL.replace("test-app", "from-ciab")
        legacy = MINIMAL.replace("test-app", "from-openhost")
        (tmp_path / "ciab.toml").write_text(ciab)
        (tmp_path / "openhost.toml").write_text(legacy)
        manifest = parse_manifest(str(tmp_path))
        assert manifest.name == "from-ciab"

    def test_parse_missing_manifest_raises_referencing_ciab(self, tmp_path):
        """The not-found error should point users at the canonical name."""
        with pytest.raises(ValueError, match="ciab.toml"):
