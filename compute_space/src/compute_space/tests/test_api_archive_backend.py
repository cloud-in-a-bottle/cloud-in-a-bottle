...
async def test_cancelled_configure_holds_lock_until_worker_exits(cfg: Any, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        ...
        drain = asyncio.create_task(wait_for_operations())
        await asyncio.sleep(0)
...
@pytest.mark.asyncio
async def test_configure_releases_operation_lock_after_success(cfg: Any, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(archive_backend, "configure_backend", lambda *_args, **_kwargs: None)
    db = sqlite3.connect(cfg.db_path)
    db.row_factory = sqlite3.Row
    try:
        await configure_archive_backend.fn(
            ConfigureArchiveRequest("bucket", "key", "secret", s3_prefix="andrew-3"), db, cfg
        )
        assert not operation_lock.locked()
    finally:
        db.close()
...
@pytest.mark.asyncio
async def test_configure_rechecks_shutdown_after_acquiring_operation_lock(
    cfg: Any, monkeypatch: pytest.MonkeyPatch
) -> None:
    checks = iter((False, True))
    monkeypatch.setattr(archive_routes, "is_shutdown_pending", lambda: next(checks))
    monkeypatch.setattr(archive_backend, "configure_backend", lambda *_args, **_kwargs: pytest.fail("configured"))
    db = sqlite3.connect(cfg.db_path)
    db.row_factory = sqlite3.Row
    try:
...
def test_manifest_requires_archive_only_matches_app_archive_true() -> None:
    ...
    ``app_archive = true`` only; cross-app access is permissive so it
...
def _archive_manifest(name: str, *, app_archive: bool, access_all_app_data: bool = False) -> AppManifest:
    return AppManifest(
        name=name,
        version="1.0",
        description="probe",
        ...
        access_all_app_data=access_all_app_data,
        app_temp_data=False,
        public_paths=["/"],
        capabilities=[],
        devices=[],
...
def test_add_app_allows_access_all_app_data_when_backend_disabled(
    apps_client: TestClient[Litestar], cookies: dict[str, str], tmp_path: Path
) -> None:
    """``access_all_app_data = true`` (without ``app_archive``) does NOT need a
    ...
    with (
        mock.patch.object(
            apps_routes,
            "parse_manifest",
            return_value=_archive_manifest("seer", app_archive=False, access_all_app_data=True),
        ),
        mock.patch.object(apps_routes, "validate_manifest", return_value=None),
...
def test_reload_app_allows_access_all_app_data_when_archive_unhealthy(
    cfg: Any, apps_client: TestClient[Litestar], cookies: dict[str, str]
) -> None:
    """access_all_app_data apps can still reload while the archive is unhealthy
