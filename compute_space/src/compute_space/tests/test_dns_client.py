"""The DNS client: record-level reads and writes, and both dispatch paths."""
...
def _stored(db: sqlite3.Connection) -> dict[tuple[str, str], list[str]]:
...
async def test_publishes_and_clears_a_challenge(tmp_path: Path) -> None:
    with _local(tmp_path) as (dns, db):
        await dns.set_records(CHALLENGE, RecordType.TXT, ["base", "wildcard"], ttl=60)
        assert sorted(_stored(db)[("_acme-challenge", "TXT")]) == ['"base"', '"wildcard"']

        await dns.delete_records(CHALLENGE, RecordType.TXT)
...
async def test_publishing_replaces_a_previous_runs_leftovers(tmp_path: Path) -> None:
    ...
    with _local(tmp_path) as (dns, db):
        await dns.set_records(CHALLENGE, RecordType.TXT, ["stale"], ttl=60)
        await dns.set_records(CHALLENGE, RecordType.TXT, ["fresh"], ttl=60)
...
async def test_challenges_carry_a_short_ttl(tmp_path: Path) -> None:
    ...
    with _local(tmp_path) as (dns, db):
        await dns.set_records(CHALLENGE, RecordType.TXT, ["tok"], ttl=60)
        records = store.all_records(db)
        assert [r.ttl for r in records if r.name == "_acme-challenge"] == [60]


@pytest.mark.asyncio
async def test_names_are_passed_through_relative_to_the_zone(tmp_path: Path) -> None:
    # The client does no name rewriting: "@" is the apex and "www" is a label, exactly as given.
    with _local(tmp_path) as (dns, db):
        await dns.set_records("@", RecordType.A, ["198.51.100.7"])
        await dns.set_records("www", RecordType.A, ["198.51.100.8"])
        assert _stored(db)[("@", "A")] == ["198.51.100.7"]
        assert _stored(db)[("www", "A")] == ["198.51.100.8"]
...
async def test_a_challenge_reaches_the_zone_file(tmp_path: Path) -> None:
    ...
    with closing(open_db(config)) as db:
        await DnsClient(db).set_records(CHALLENGE, RecordType.TXT, ["tok"], ttl=60)
    assert '_acme-challenge   60  IN TXT  "tok"' in config.coredns_zonefile_path.read_text()


@pytest.mark.asyncio
async def test_a_write_lands_in_every_zone(tmp_path: Path) -> None:
    # The zones are aliases for one space, so a record is stored once and rendered into all of
    # them -- there is no such thing as writing to only some.
    config = seeded_dns_config(tmp_path, Domain(ZONE, tls=True), Domain("host.example.org", tls=True))
    with closing(open_db(config)) as db:
        await DnsClient(db).set_records(CHALLENGE, RecordType.TXT, ["tok"], ttl=60)
        assert _stored(db)[("_acme-challenge", "TXT")] == ['"tok"']
    for path in (config.coredns_zonefile_path, config.zones_dir / "host.example.org.zone"):
        assert '_acme-challenge   60  IN TXT  "tok"' in path.read_text()
...
async def test_the_router_claims_only_the_records_it_writes(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    recorder = _Recorder({"/records/set": _write_ok(ALL_ZONES)})
    await _remote(recorder, monkeypatch).set_records(CHALLENGE, RecordType.TXT, ["tok"], ttl=60)
...
@pytest.mark.asyncio
async def test_every_write_addresses_all_zones(monkeypatch: pytest.MonkeyPatch) -> None:
    # Including against a connector, which may well be zone-aware: the router has no zone to name
    # now that it never asks which ones exist.
    recorder = _Recorder({"/records/set": _write_ok(ALL_ZONES)})
    await _remote(recorder, monkeypatch).set_records(CHALLENGE, RecordType.TXT, ["tok"], ttl=60)

    body = recorder.body()
    assert body["zone"] == ALL_ZONES
    assert body["records"] == [{"name": "_acme-challenge", "type": "TXT", "ttl": 60, "data": "tok"}]
...
async def test_clearing_omits_data_entirely(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    recorder = _Recorder({"/records/delete": _write_ok(ALL_ZONES)})
    await _remote(recorder, monkeypatch).delete_records(CHALLENGE, RecordType.TXT)
...
async def test_a_per_zone_failure_is_raised_rather_than_read_as_success(monkeypatch: pytest.MonkeyPatch) -> None:
    # A connector may report 207 for a partial fan-out; any zone that failed means the record is
    # not being served there, and reading that as success would silently drop a challenge.
    recorder = _Recorder(
...
async def test_an_unreachable_provider_is_an_error_not_a_crash(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ServiceCallError, match="unreachable"):
        await _remote(recorder, monkeypatch, refuse).set_records(CHALLENGE, RecordType.TXT, ["tok"], ttl=60)
