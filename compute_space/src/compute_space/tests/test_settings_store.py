...
def test_set_get_delete(tmp_path: Path) -> None:
    cfg = _cfg(tmp_path)
    assert get_setting(cfg, "claim_token") is None
    set_setting(cfg, "claim_token", "abc123")
    assert get_setting(cfg, "claim_token") == "abc123"
    # upsert overwrites
    set_setting(cfg, "claim_token", "def456")
    assert get_setting(cfg, "claim_token") == "def456"
    delete_setting(cfg, "claim_token")
    assert get_setting(cfg, "claim_token") is None
