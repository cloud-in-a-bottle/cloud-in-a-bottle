...
def create_app(config: Config, dns: InternalDnsProvider | None = None) -> ASGIApp:
    ...
    litestar_app = Litestar(
        ...
        dependencies={
            ...
            # The running provider itself, so a route that changes the domain set can tell it,
            # rather than reaching for whichever one a module global happens to hold.  None when
            # the router isn't serving DNS (`coredns_enabled` off, or a connector app is default).
