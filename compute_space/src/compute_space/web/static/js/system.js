...
// visible at a glance. Data comes from the same diagnostics endpoints the
// Diagnostics page uses: per-app live stats + host totals.
...
function updateResourceUsage() {
  Promise.all([
    fetch(config.appsUrl, {credentials: 'same-origin'}).then(function(r) { return r.json(); }),
    fetch(config.diagnosticsSystemUrl, {credentials: 'same-origin'}).then(function(r) { return r.json(); }),
  ]).then(function(res) {
    var apps = res[0] || [];
    var pressure = (res[1] && res[1].resource_pressure) || null;
    renderResourceUsage(apps, pressure);
  }).catch(function() {
