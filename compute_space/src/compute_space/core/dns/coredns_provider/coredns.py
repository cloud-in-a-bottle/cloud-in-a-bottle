...
a restart — which is why ``InternalDnsProvider`` owns the process rather than this module.  Record
reads and writes go through ``operations``; here it is only rendering and the child process.
"""
...
def public_dns_zones(settings: DnsSettings, zones: Sequence[ManagedZone]) -> tuple[DnsZone, ...]:
    ...
    The primary keeps the legacy ``zonefile`` path; every other zone gets its own under ``zones/``,
    since a zone's ACME ``_acme-challenge`` records have to land in its own file.
    """
...
def write_coredns_config(
    ...
    *,
    ...
    for zone in zones:
        write_zone_file(zone, settings, db, default_ttl)
    ...
    for zone in zones if container_gateway_ip else ():
        ...
        zone.container_zonefile_path.write_text(
            _jinja_env.get_template("zonefile_container").render(
                zone_domain=zone.domain, gateway_ip=container_gateway_ip, serial=_next_serial(settings, db)
            )
        )
...
def write_zone_file(
    ...
    """Generate a zone file from the stored records, overwriting it.
    ...
    with, the addresses routing the space included, is a stored record.
    ...
    content = _jinja_env.get_template("zonefile").render(
        ...
        serial=_next_serial(settings, db),
        default_ttl=default_ttl,
        records=store.all_records(db) if db is not None else [],
    )
...
def _next_serial(settings: DnsSettings, db: sqlite3.Connection | None) -> int:
    ...
    the second change would never be picked up.  With nowhere to persist one -- a render with no
    DB, which only a test does -- the clock is all there is.
    """
    if db is None:
        return int(time.time())
    previous = settings.get_serial(db) or 0
...
async def start_coredns(
    ...
    *,
    ...
    write_coredns_config(zones, settings, db=db, default_ttl=default_ttl)
