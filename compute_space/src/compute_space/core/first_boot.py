...
exactly once.  On an old instance there is no ``first_boot.toml`` — the primary domain is seeded from
the config-file ``zone_domain`` and the claim token from its legacy file.
...
def seed_first_boot(config: Config) -> None:
    ...
    (primary domain) and the legacy claim-token file (token).
    ...
    with closing(get_db()) as db:
        if fb is not None:
            ...
            domain_store.seed_domains(db, primary, [])
        else:
            domain_store.seed_domains_from_legacy(config, db)
        ...
        if not _owner_exists(config) and settings_store.get_setting(db, settings_store.CLAIM_TOKEN_KEY) is None:
