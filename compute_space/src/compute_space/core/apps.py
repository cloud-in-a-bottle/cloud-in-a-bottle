...
class AppError(Exception):
    """App-lifecycle failure carrying an operator-facing message.
    The API layer maps each subclass onto an HTTP status code."""


class AppInputError(AppError):
    """The caller supplied an invalid app id or argument."""


class AppNotFoundError(AppError):
    """No app row exists for the given id."""


class AppConflictError(AppError):
    """The app's current status refuses this operation (removing/reloading)."""


class AppUnavailableError(AppError):
    """A backend the operation needs (archive mount, worker spawn) is down; retryable."""
...
def resolve_app(app_id: str, db: sqlite3.Connection) -> sqlite3.Row:
    """Validate app_id format and load the app row."""
    if not is_valid_app_id(app_id):
        raise AppInputError("Invalid app_id")
...
def is_removing(app_row: sqlite3.Row | None) -> bool:
...
async def pin_refless_to_landed_branch(repo_url: str | None, repo_path: str) -> str | None:
...
async def read_app_git_info(repo_path: str | None) -> tuple[str | None, str | None, bool | None]:
...
@attr.s(auto_attribs=True, frozen=True)
class UpdateReview:
    """Settings/permission changes an update pulled in that need explicit owner
    approval before the reload may proceed."""

    settings_changed: list[dict[str, Any]]
    permissions_required: list[dict[str, Any]]


def gate_update_review(
    app_id: str,
    repo_path: str,
...
@attr.s(auto_attribs=True, frozen=True)
class ReloadOutcome:
    """What reload_app decided; kind is one of 'started', 'no_repo', 'pull_failed',
    'oauth_required', or 'review_required'. The /reload_app route maps these onto responses."""

    kind: str
    app_name: str
    authorize_url: str | None = None
    review: UpdateReview | None = None
...
async def reload_app(
    ...
    """Optionally pull latest code, gate unapproved manifest changes, then claim the
    row and spawn the reload worker. Raises AppError subclasses on refusals."""
    app_row = resolve_app(app_id, db)
    if is_removing(app_row):
        raise AppConflictError("App is being removed")
    app_name = app_row["name"]
    ...
    with open(log_file, "a") as lf:
        ...
        if update or continue_oauth:
            if not app_row["repo_path"] or not os.path.isdir(os.path.join(app_row["repo_path"], ".git")):
                ...
                db.commit()
                return ReloadOutcome(kind="no_repo", app_name=app_name)
            ...
            if not pull_ok and is_github_repo_url(repo_url):
                ...
                try:
                    ...
                    return ReloadOutcome(kind="oauth_required", app_name=app_name, authorize_url=e.authorize_url)
                except RuntimeError as e:
                    ...
                    db.commit()
                    return ReloadOutcome(kind="pull_failed", app_name=app_name)
                lf.flush()
                pull_ok, pull_err = await asyncio.to_thread(
                    git_pull,
                    app_row["repo_path"],
            ...
            # every future update.
            pinned = await pin_refless_to_landed_branch(repo_url, app_row["repo_path"])
            if pinned:
    ...
    if update or continue_oauth:
        ...
        if review is not None:
            ...
            return ReloadOutcome(kind="review_required", app_name=app_name, review=review)
    ...
    if cursor.rowcount == 0 and not continue_oauth:
        raise AppConflictError("App is already reloading")
    ...
    return ReloadOutcome(kind="started", app_name=app_name)
...
def wipe_data_restart(app_id: str, db: sqlite3.Connection, config: Config) -> None:
    """Claim the app row as 'building' and spawn the wipe/restart worker.
    Raises AppError subclasses on refusals."""
    app_row = resolve_app(app_id, db)
    if is_removing(app_row):
        raise AppConflictError("App is being removed")
    if app_row["status"] in ("building", "starting"):
        raise AppConflictError("App is already restarting")
    ...
    try:
        ...
        db.commit()
        raise AppUnavailableError("Could not start wipe/restart worker; try again.") from exc
...
def rename_app_storage_dirs(config: Config, old_name: str, new_name: str, archive_dir: str) -> str | None:
