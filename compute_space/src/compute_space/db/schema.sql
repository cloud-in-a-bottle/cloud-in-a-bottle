...
-- first_boot.toml, then authoritative).  Exactly one row is the primary (is_primary=1), enforced by
-- the partial unique index below; the primary (read live via domains.primary_domain(db)) is the
-- canonical domain used by background tasks and the OPENHOST_ZONE_DOMAIN handed to apps.
-- cert_status mirrors the /api/domains acquisition state machine.
